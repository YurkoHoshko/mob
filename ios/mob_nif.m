...
// ── NIF: resolve_ipv4/1 ─────────────────────────────────────────────────────
// Resolve through the platform DNS stack in-process. This avoids OTP's
// inet_gethost port program, which iOS cannot execute from the app bundle.
...
static ERL_NIF_TERM nif_resolve_ipv4(ErlNifEnv* env, int argc, const ERL_NIF_TERM argv[]) {
    ...
    if (rc != 0 || result == NULL) {
        const char *message = rc != 0 ? gai_strerror(rc) : "no IPv4 address";
